...
class ParentClass:
    ...
    def on_select(self, event=None):
        ...
        self.portName = event.widget.get()
        st = self.portName.find('/')
        ed = self.portName.find(' ')
        self.portName = self.portName[st:ed]
    ...
    def plotting(self):
        if self.start == True:
            if self.ser.in_waiting > 0:
                self.tmp = self.ser.read().decode("Ascii")
                self.string_data = self.string_data + self.tmp

                start = self.string_data.find("START")
                if(start != -1):
                    end = self.string_data[start:].find("STOP")
                    if(end != -1):
                        rawData = self.string_data[start+11:end]
                        self.string_data = ''
                        ...
                        try:
                            self.frontData.append(
                                float(dataSplit[0]))  # Data ชุดหน้า
                            self.backData.append(
                                float(dataSplit[1]))  # Data ชุดหลัง
