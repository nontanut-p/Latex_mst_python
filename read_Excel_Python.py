...
def readData():
    ...
    for row_cells in sheet.iter_rows(min_row=2, max_row=sheet.max_row):
        try:
            timeMT.append(row_cells[ColNames['Time MST']].value)
            laserDis.append(row_cells[ColNames['Load cell']].value)
...
def plotXY():
    ...
    ax[0].legend(["FORCE 1",  "FORCE: : MA " + str(short),
                 "FORCE :  MA " + str(long)], loc="lower right")
    ...
    ax[1].legend(["Force 1 "], loc="lower right")
